...
try{
    def translatedServiceCode = ACCESSGW.COMMAND.serviceCode.toString()
    logger.info("test ${ACCESSGW.COMMAND.serviceCode} $map")
    if (map.containsKey(translatedServiceCode)) {
        logger.info("vao ${ACCESSGW.COMMAND.serviceCode} $map")
        translatedServiceCode = map.get(translatedServiceCode)
        logger.info("translate ${ACCESSGW.COMMAND.serviceCode} -> $translatedServiceCode")
    }

    if(ACCESSGW.COMMAND.regType == "1"){
        error_desc = SPSClient.createSDPService(
                ACCESSGW.COMMAND.msisdn,
                translatedServiceCode,
